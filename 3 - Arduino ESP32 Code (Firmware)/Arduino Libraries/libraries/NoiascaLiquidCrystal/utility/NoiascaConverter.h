/*
   ...
   by noiasca
   2021-04-04 updated Cyrillic converter, SPLC780D1_002A Cyrillic, changed to UTF-8 w/o BOM 
...
// this table also contain some normalization of characters if not supported by the ROM
...
const byte NOPRINT = 0;
const byte PRINT = 1;
const byte PRINT2 = 2;
const byte ADDE = 4;


// enum class NOIASCA_LCD_ROM_TYPE{HD44780_A00, SPLC780D1_003A, ST7070};
...
uint8_t convert(uint32_t &special, uint8_t &value);                  // HD44780 Ä -> A. Converts large german umlauts to letters without diareses.
uint8_t convert_ae(uint32_t &special, uint8_t &value);               // HD44780 Ä -> Ae. Extends large letter umlauts with a small letter e.
...
uint8_t convert_small(uint32_t &special, uint8_t &value);            // HD44780 Ä -> ä. Converts large german umlauts to their small letters. That's not so bad if you compare the Ü/ü or Ö/ö and large letter Ä are seldom used!
uint8_t convert_special(uint32_t &special, uint8_t &value);          // HD44780 Ä -> Ä. Uses custom characters to get a precise replacements
uint8_t convert_ST7070(uint32_t &special, uint8_t &value);           // ST7070 only
uint8_t convert_ST7070_ext(uint32_t &special, uint8_t &value);       // ST7070 only
uint8_t convert_SPLC780D1_002A(uint32_t &special, uint8_t &value);   // SPLC780D1 with 003A ROM only (Cyrillic)
uint8_t convert_SPLC780D1_003A(uint32_t &special, uint8_t &value);   // SPLC780D1 with 003A ROM only (European)
//uint8_t convert_HD44780A02(uint32_t &special, uint8_t &value);     // HD44780 with ROM A02 - European (latin) and some cyrillic letters - tbd
